...
class AdversarialVAE(nn.Module):
    ...
    def __init__(self, weight):
        ...
        #================ Encoder model =============#
        self.encoder = nn.GRU(mconfig.embedding_size, mconfig.hidden_dim, batch_first=True, bidirectional=True)
        # content latent embedding
        self.content_mu = nn.Linear(2*mconfig.hidden_dim, mconfig.content_hidden_dim)
        self.content_log_var = nn.Linear(2*mconfig.hidden_dim, mconfig.content_hidden_dim)
        # style latent embedding
        self.style_mu = nn.Linear(2*mconfig.hidden_dim, mconfig.style_hidden_dim)
        self.style_log_var = nn.Linear(2*mconfig.hidden_dim, mconfig.style_hidden_dim)

        #=============== Discriminator/adversary============#
        self.style_disc = nn.Linear(mconfig.content_hidden_dim, mconfig.num_style)
        self.content_disc = nn.Linear(mconfig.style_hidden_dim, mconfig.content_bow_dim)

        #=============== Classifier =============#
        self.content_classifier = nn.Linear(mconfig.content_hidden_dim, mconfig.content_bow_dim)
        self.style_classifier = nn.Linear(mconfig.style_hidden_dim, mconfig.num_style)

        #=============== Decoder =================#
        # Note: input embeddings are concatenated with the sampled latent vector at every step
        self.decoder = nn.GRUCell(mconfig.embedding_size + mconfig.generative_emb_dim, mconfig.hidden_dim)
        ...
        self.avg_style_emb = {
            0: torch.zeros(mconfig.style_hidden_dim, device=torch.device("cuda")),
            1: torch.zeros(mconfig.style_hidden_dim, device=torch.device("cuda"))
        }
    ...
    def forward(self, sequences, seq_lengths, style_labels, content_bow, iteration, last_epoch):
        ...
        # It requires the sentences to be sorted in descending order to take full advantage
        ...
        packed_seqs = pack_padded_sequence(embedded_seqs, lengths=seq_lengths.cpu(), batch_first=True)
        ...
        style_emb_mu, style_emb_log_var = self.get_style_emb(sentence_emb)
        ...
        content_disc_loss = self.get_content_disc_loss(content_disc_preds, content_bow)
        ...
        style_disc_loss = self.get_style_disc_loss(style_disc_preds, style_labels)
        ...
        # Style space
        style_kl_loss = self.get_kl_loss(style_emb_mu, style_emb_log_var)

        if iteration < mconfig.kl_anneal_iterations:
            style_kl_loss = self.get_annealed_weight(iteration, mconfig.style_kl_lambda) * style_kl_loss

        # Content space
        content_kl_loss = self.get_kl_loss(content_emb_mu, content_emb_log_var)
        
        if iteration < mconfig.kl_anneal_iterations:
            content_kl_loss = self.get_annealed_weight(iteration, mconfig.content_kl_lambda) * content_kl_loss

        #=============== reconstruction ================#

        reconstructed_sentences = self.generate_sentences(sequences, generative_emb)
    ...
    def generate_sentences(self, input_sentences, latent_emb, inference=False):
        ...
        if not inference:
            ...
            sos_token_tensor = torch.cuda.LongTensor(
                [gconfig.predefined_word_index['<sos>']]).unsqueeze(0).repeat(len(input_sentences), 1)
            ...
            for idx in range(mconfig.max_seq_len):
                ...
                hidden_states = self.decoder(
                    words, hidden_states[0:len(input_sentences)])
